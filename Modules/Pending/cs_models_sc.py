...
def ifft_layer(kspace_2channel):
    ...
    image_complex = tf.ifft2d(kspace_complex)
    
    # expand channels to tensorflow/keras format
    #real = K.tf.expand_dims(K.tf.real(image_complex),-1)
    real = tf.expand_dims(tf.real(image_complex),-1)
    #imag = K.tf.expand_dims(K.tf.imag(image_complex),-1)
    imag = tf.expand_dims(tf.imag(image_complex),-1)
...
def DC_block(rec,mask,sampled_kspace,channels,kspace = False):
    ...
    if kspace: # if in kSpace leave the same 
        rec_kspace = rec
    else:
        rec_kspace = Lambda(fft_layer)(rec) # convert data to k Space 
    rec_kspace_dc =  Multiply()([rec_kspace,mask]) # mask kSpace Values  
    rec_kspace_dc = Add()([rec_kspace_dc,sampled_kspace]) # add the values that have been acquired in the points still existing after masking 
    return rec_kspace_dc #kSpace after Data Consistency layer 
...
def deep_cascade_flat_unrolled(depth_str = 'ikikii', H=256,W=256,depth = 5,kshape = (3,3), nf = 48,channels = 2):
    ...
    for ii in depth_str:
        ...
        if ii =='i':
            # Add IFFT
            layers.append(Lambda(ifft_layer)(layers[-1]))   # returns image_complex_2channel
            kspace_flag = False 
        # Add CNN block
        layers.append(cnn_block(layers[-1],depth,nf,kshape,channels)) # Adds 5(=depth) layers of Conv2D 

        # Add DC block
        layers.append(DC_block(layers[-1],mask,inputs,channels,kspace=kspace_flag)) # Adds DC layer: you keep kSpace data after you have masked the kSpace 
        kspace_flag = True
