...
def CreateNuffts(nsamples=256, nspokes=256, dtype=torch.complex64):
    ...
    adjnufft_ob = tkbn.KbNufftAdjoint(im_size=im_size, grid_size=grid_size, ).to(
        dtype)  # Reconstructs Radial kSpace to IMG
...
def Rad2DUndMask(kdata, nsamples=256, nspokes=256, und=0.8):
    # undersampling = 0.8 means we get to keep 80% of the image.
...
def kSpaceR_US(kSpaceR, Masks, res=False):
    ...
    if res:
        kSpace_Us = np.reshape(kSpace_Us,
                               (kSpace_Us.shape[0], int(np.sqrt(kSpace_Us.shape[1])), int(np.sqrt(kSpace_Us.shape[1]))))
...
def RecImgAll(us_kdata, traj):
    images_us = np.zeros((us_kdata.shape[0], int(np.sqrt(us_kdata.shape[-1])), int(np.sqrt(us_kdata.shape[-1]))),
                         dtype=np.complex64)
    ...
    for i in range(us_kdata.shape[0]):
        ...
        if i % 100 == 0:
            print('done with {} slices'.format(i))
    return images_us
